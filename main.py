...
from GF256 import GF256, GF256Errors
...
class KeyPairGenerator:
    ...
    def __init__(self, n=20, u=2, k=7, save=''):
        """
        ...
        self.L1.start_generators(10)

        self.L2 = Affine(self.n, self.k)
        self.L2.start_generators(10)
    ...
    def generate_random_matrix(self, x, y, k):
        """
        ...
        for i in range(x):
            for j in range(y):
                mat[i][j] = self.generate_random_element()
        return mat
    ...
    def generate_vinegars(self):
        ...
        rnum = self.generate_random_element()
    ...
    def generate_coefficients(self):
        ...
        for _i in range(self.u - 1):
            ...
            for i in range(ol):
                ...
                alphas = self.generate_random_matrix(self.v[_i], self.v[_i], self.k)
                betas = self.generate_random_matrix(self.v[_i + 1] - self.v[_i], self.v[_i], self.k)
                gammas = self.generate_random_matrix(1, self.v[_i + 1], self.k)
                etas = self.generate_random_element()
        ...
        for _i in range(ol):
            ...
            for i in range(vl):
                for j in range(vl):
                    ...
                    polynomial.quadratic[pcount + _i] = GF256().add_matrices(polynomial.quadratic[pcount + _i],
                                                                             GF256().multiply_vectors(temp, self.L2[j]))
                    ...
                    polynomial.constant[pcount + _i] = GF256().add(polynomial.constant[pcount + _i],
                                                                   GF256().multiply(temp, self.b2[j]))
            ...
            for i in range(vl + ol):
                ...
                polynomial.linear[pcount + _i] = GF256().add_vectors(temp, polynomial.linear[pcount + _i])
                polynomial.constant[pcount + _i] = GF256().add(polynomial.constant[pcount + _i],
                                                               GF256().multiply(coefficients[_i]['gammas'][0][i],
                                                                                self.b2[i]))

            # Add Eta
            polynomial.constant[pcount + _i] = GF256().add(polynomial.constant[pcount + _i],
                                                           coefficients[_i]['etas'][0])
    ...
    def generate_publickey(self, save=''):
        ...
        self.polynomial.quadratic = [[[0] * self.n for _ in range(self.n)] for _ in range(self.n - self.v[0])]
        self.polynomial.linear = [[0] * self.n for _ in range(self.n - self.v[0])]
        ...
        temp_quadratic = [[[0] * self.n for _ in range(self.n)] for _ in range(self.n - self.v[0])]
        temp_linear = [[0] * self.n for _ in range(self.n - self.v[0])]
        temp_constant = [0] * (self.n - self.v[0])

        for i in range(self.n - self.v[0]):
            for j in range(len(self.L1)):
                temp_quadratic[i] = GF256().add_matrices(temp_quadratic[i],
                                                         GF256().multiply_matrix_scalar(self.polynomial.quadratic[j],
                                                                                        self.L1[i][j]))
                temp_linear[i] = GF256().add_vectors(temp_linear[i], GF256().multiply_scalar_vector(self.L1[i][j],
                                                                                                    self.polynomial.linear[
                                                                                                        j]))
                temp_constant[i] = GF256().add(temp_constant[i],
                                               GF256().multiply(self.L1[i][j], self.polynomial.constant[j]))
            ...
            for j in range(len(self.polynomial.quadratic[i])):
                for k in range(j, len(self.polynomial.quadratic[i][j])):
                    if j == k:
                        ...
                        compact_quads[-1].append(
                            GF256().add(self.polynomial.quadratic[i][k][j], self.polynomial.quadratic[i][j][k]))
    ...
    def generate_targets(self, n, v0, k, message):
        """
    ...
    def sign(self, keyFile, msgFile):
        ...
        with open(msgFile, 'r') as mFile:
            message = mFile.read()
            y = self.generate_targets(privKey.n, len(privKey.F_layers[0][0]['alphas'][0]), privKey.k,
                                      message)
            if args.v:
                print("Message from file: ", y)
        ...
        while True:
            try:
                ...
                for i in range(len(privKey.F_layers[0][0]['alphas'][0])):
                    x.append(self.generate_random_element())
                ...
                for layer in range(privKey.layers):
                    ...
                    for i in range(ol):
                        for j in range(vl):
                            for k in range(vl):
                                consts[i] = GF256().add(consts[i],
                                                        GF256().multiply(
                                                            GF256().multiply(privKey.F_layers[layer][i]['alphas'][j][k],
                                                                             x[j]),
                                                            x[k])
                                                        )
                        ...
                        for j in range(ol):
                            for k in range(vl):
                                equations[i][j] = GF256().add(equations[i][j], GF256().multiply(
                                    privKey.F_layers[layer][i]['betas'][j][k], x[k]))

                        for j in range(ol + vl):
                            if j < vl:

                                consts[i] = GF256().add(consts[i],
                                                        GF256().multiply(privKey.F_layers[layer][i]['gammas'][0][j],
                                                                         x[j]))
                            else:

                                equations[i][j - vl] = GF256().add(equations[i][j - vl],
    ...
    def verify(self, keyFile, signature, msgFile):
        """
        ...
        with open(msgFile, 'r') as mFile:
            ...
            if args.v:
                print(message)
            y = KeyPairGenerator().generate_targets(pubKey.n, pubKey.v0, pubKey.k, message)
    ...
    def save_signature(self, signature, sfile='rSignature'):
        with open(sfile, 'wb') as opFile:
            dill.dump(signature, opFile)

    def load_signature(self, sfile='rSignature'):
        with open(sfile, 'wb') as opFile:
...
if __name__ == '__main__':
    ...
    while correct_flag == 0:
        myKeyObject = KeyPairGenerator(save='rainbowTest')
        try:
            start = dt.now()
            signature = KeyPairGenerator().sign('rainbowTest.pem', 'test.txt')
            end = dt.now()
            if args.v:
                print("Signed in", end - start, "seconds")
        except Exception as _:
            print("Keys sign-check failed, regenerating.")
            continue

        try:
            start = dt.now()
            print("Signature verification :", KeyPairGenerator().verify('rainbowTest.pub', signature, 'test.txt'))
            end = dt.now()
            if args.v:
                print("Verified signature in", end - start, "seconds")
        except Exception as _:
            print("Keys verify-check failed, regenerating.")
            continue
