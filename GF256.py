...
class GF256:
    ...
    def get(self):
        ...
        """
        return secrets.choice(self.exponenets)
    ...
    def multiply(self, x, y):
        ...
        if x == 0 or y == 0:
            ...
            return GF256.exponenets[(self.logarithms[x] + self.logarithms[y]) % 255]
    ...
    def get_inverse(self, x):
        ...
        if x == 0:
            ...
            return self.exponenets[255 - self.logarithms[x]]

    def lower_zero_matrix(self, mat, inverse):
        """
        ...
        for k in range(len(mat) - 1):
            for i in range(k + 1, len(mat)):
                factor = mat[i][k]
                factor2 = self.get_inverse(mat[k][k])
                if factor2 == 0:
                    raise GF256Errors("Matrix not invertible")

                for j in range(k, length):
                    temp = self.multiply(mat[k][j], factor2)
                    temp = self.multiply(factor, temp)
                    mat[i][j] = self.add(mat[i][j], temp)

    def upper_zero_matrix(self, mat):
        """
        ...
        for i in range(len(m1)):
            for j in range(len(m2)):
                for k in range(len(m2[0])):
                    temp = self.multiply(m1[i][j], m2[j][k])
                    ret[i][k] = self.add(ret[i][k], temp)
    ...
    def multiply_matrix_vector(self, m, v):
        ...
        for i in range(len(m)):
            for j in range(len(v)):
                temp = self.multiply(m[i][j], v[j])
                ret[i] = self.add(ret[i], temp)  # CHANGE
    ...
    def add_vectors(self, v1, v2):
        ...
        for i in range(len(v1)):
            ret[i] = self.add(v1[i], v2[i])

        return ret

    def multiply_scalar_vector(self, s, v):
        """
    ...
    def multiply_vectors(self, v1, v2):
        ...
        ret = [[0] * len(v1) for _ in range(len(v2))]
        for i in range(len(v1)):
            for j in range(len(v2)):
                ret[i][j] = self.multiply(v1[i], v2[j])

        return ret

    def multiply_matrix_scalar(self, m, s):
        """
    ...
    def add_matrices(self, m1, m2):
        ...
        for i in range(len(m1)):
            ...
            for j in range(len(m1[0])):
                temp = self.add(m1[i][j], m2[i][j])
                ret[-1].append(temp)

        return ret

    def substitute(self, m1, m2):
        """
        Backward substitution method to find x given m1 * x = m2
        """
        temp = self.get_inverse(m1[len(m1) - 1][len(m1) - 1])
        if temp == 0:
            raise GF256Errors("Equations cannot be solved!")

        m2[len(m1) - 1] = self.multiply(m1[len(m1) - 1][len(m1)], temp)

        for i in range(len(m1) - 2, -1, -1):
            _temp = m1[i][len(m1)]

            for j in range(len(m1) - 1, i, -1):
                temp = self.multiply(m1[i][j], m2[j])
                _temp = self.add(_temp, temp)

            temp = self.get_inverse(m1[i][i])

            if temp == 0:
                raise GF256Errors("Equations cannot be solved!")

            m2[i] = self.multiply(_temp, temp)

        return m2

    def solve_equation(self, m1, m2):
        """
        ...
        temp = [[0] * (len(m1) + 1) for _ in range(len(m1))]
        ret = [0] * len(m1)
        ...
        for i in range(len(m2)):
            temp[i][len(m2)] = self.add(m2[i], temp[i][len(m2)])

        self.lower_zero_matrix(temp, False)
        ret = self.substitute(temp, ret)

        return ret

    def find_inverse(self, mat):
        """
        Calculate the inverse of a given matrix
        """
        try:
            temp = [[0] * (2 * len(mat)) for _ in range(len(mat))]
            ...
            self.lower_zero_matrix(temp, True)

            for i in range(len(temp)):
                factor = self.get_inverse(temp[i][i])
                for j in range(i, len(temp) * 2):
                    temp[i][j] = self.multiply(temp[i][j], factor)

            self.upper_zero_matrix(temp)
            ret = [[0] * len(temp) for _ in range(len(temp))]
            for i in range(len(temp)):
